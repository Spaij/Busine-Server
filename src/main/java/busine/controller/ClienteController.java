...
import busine.repository.ClienteRepository;
import busine.repository.OrganizacaoRepository;
...
public class ClienteController {
    ...
    @RequestMapping(value = "/organizacao/{codOrganizacao}", method = RequestMethod.GET)
    public ResponseEntity<?> getOrganizacaoById(@PathVariable("codOrganizacao") Long codOrganizacao) {
        logger.info("Buscando organizacao com código {}", codOrganizacao);
        Organizacao organizacao = organizacaoRepository.findOne(codOrganizacao);
        if (organizacao == null) {
            logger.error("Organizacao com código {} nao encontrada.", codOrganizacao);
            return new ResponseEntity(new CustomErrorType("Organizacao com código " + codOrganizacao
                    + " nao encontrada"), HttpStatus.NOT_FOUND);
        }
    ...
    public ResponseEntity<?> cadastrarOrganizacao(@RequestBody Organizacao organizacao, UriComponentsBuilder ucBuilder) {
        logger.info("Cadastrando organização : {}", organizacao.getNomeDaOrganizacao());
 
        if (organizacaoRepository.findByNomeDaOrganizacaoLike(organizacao.getNomeDaOrganizacao()) != null) {
            logger.error("Nao foi possivel cadastrar. Uma organizacao com a nome {} ja existe", organizacao.getNomeDaOrganizacao());
            return new ResponseEntity(new CustomErrorType("Nao foi possivel cadastrar. Uma organização com o nome " + organizacao.getNomeDaOrganizacao() + " ja existe."),HttpStatus.CONFLICT);
        }
        
        Organizacao org = organizacaoRepository.save(organizacao);
 
        HttpHeaders headers = new HttpHeaders();
        headers.setLocation(ucBuilder.path("/organizacao/{codOrganizacao}").buildAndExpand(org.getCodOrganizacao()).toUri());
    ...
    public ResponseEntity<?> getClienteByNome(@RequestParam(value="nome") String nome) {
        ...
        if (cliente == null) {
            ...
            return new ResponseEntity(new CustomErrorType("Cliente com a nome " + nome 
                    + " nao encontrado"), HttpStatus.NOT_FOUND);
        }
        return new ResponseEntity<Cliente>(cliente, HttpStatus.OK);
    }
